...
def load_data():
    import glob
    # Procura qualquer arquivo .csv dentro de data/raw/
    files = glob.glob("data/raw/*.csv") + glob.glob("data/raw/*")
    for f in files:
        if "healthcare" in f.lower() or f.endswith(".csv"):
            try:
                return pd.read_csv(f)
            except Exception:
                continue
    st.error("Arquivo CSV não encontrado em data/raw/")
    return None
...
if df is not None:
    ...
    with tab3:
        ...
        if "(a)" in sim_choice:
            ...
            cum_means = [minhastats.mean(rolls[:i].tolist()) for i in range(1, n_reps + 1)]
    ...
    with tab5:
        ...
        y_hat = b0 + b1 * x_pred
        st.success(f"**Valor Predito para {y_var} ($\hat{{Y}}$):** {y_hat:.2f}")
